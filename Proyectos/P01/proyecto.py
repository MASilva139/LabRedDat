...
def app():
    ...
    st.write(
        ...
        Cabe a resaltar que tanto la gráfica generada a partir de la función ``stats.binom()``, como la función ``def``, dan el mismo resultado; es decir, generan la misma gráfica de barras y de ajuste, por lo que se optó solamente a utiizar una de estas funciones para realizar los fits. De igual manera, por comodidad y sobre todo por estética, se prefirio utilizar la gráfica generada por la librería de ``Plotly.Express``, dado que no solamente presenta la gráfica de la función de ajuste con una curva suave, sino que también permite observar, por medio de un zoom, de mejor manera la función de ajuste. De igual manera se puede observar, por medio de las dos gráficas, que los datos del lanzamiento de las monedas muestra el comportamiento esperado, descrito por el modelo de distribución binomial.
