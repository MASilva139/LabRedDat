...
def app():
    ...
    with c2: 
        ...
        if opt == "Proyecto":
            ...
            st.write(
            r"""
                En los últimos años se ha desarrollado la implementación de la inteligencia artificial en varias ramas de la ciencia; siendo el machine learning una rama de la inteligencia artificial que ha permitido que las máquinas aprendan de manera "independiente" sin la necesidad previa de ser programadas para ello. Una de las ramas en donde se emplea el machine learning es en el análisis de datos, dado que en algunos casos tienden a ser una enorme cantidad de datos complejos a analizar, con lo cual a partir de un modelo se puede realizar un modelo rápido y automático se pueden generar resultados más rápidos y presisos.
                
                Es por ello que el objetivo en la presente práctica es entrenar un vector de peso para clasificar los datos ingresados por medio de un data set, y observar tan bien se entrenó el vector de peso respecto al los datos ingresados, para un sistema de dos o mas dimensiones de datos. Por tanto, para ello se definió un vector de peso a entrenar $$\vec{W}_{training}$$, que depende del promedio de la función del descenso del gradiente $$\overline{\nabla loss^{2}}$$, y a partir de 100 iteraciones se obtiene el valor del vector de peso.
                
                Se determinó que el vector de peso entrenado $$\vec{W}_{training}$$ no difiere por mucho, con los datos utilizados, del vector de peso $$\vec{W}_{target}$$. Es por ello que al clasificar un data set con el training se tienen pocos desaciertos, debido a lo bien entrenado que está el vector de peso.
            """
            )
